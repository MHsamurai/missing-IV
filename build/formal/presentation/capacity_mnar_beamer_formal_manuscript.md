...
本日は「欠測IVと潜在変数モデリングによる非無作為欠測の識別」について報告します。まず多変量MNARの識別問題を定義し、その具体的なマーケティング実証として、有限の公開枠をもつオンラインレビューを扱います。中心は、平均評価や極端評価率をtarget-specific bridgeによりノンパラメトリックに回復し、その性能をoracle評価と比較することです。

## Slide 02: Motivation：多変量MNARからマーケティング実証へ

欠測値、選択、反実仮想の非観測はマイクロデータで頻繁に生じます。MNARでは未観測値そのものが観測確率へ入るため、観測集団だけの分布は母集団分布を一般に表しません。選択だけを動かす通常のIVだけでは分布全体の点識別は困難ですが、d'Haultfoeuilleは、アウトカムを条件づけた後に選択と独立になるmissing IVと完備性により、欠測アウトカム分布をノンパラメトリックに識別しました。近年はself-censoringやno self-censoringという多変量MNARも扱われています。本研究は、レビュー候補が有限の公開枠を競う状況を、自己値と他項目の双方に依存する多変量MNARとして定義します。

## Slide 03: 多変量MNARでは、項目jの公開が評価ベクトル全体に依存する

図の左側が本研究の基本的な多変量MNARです。各項目の公開指標Dは自己の評価Yだけでなく、他項目の評価にも依存します。容量制約のため、ある項目が選ばれると別の項目が押し出されるcross-item crowd-outが生じます。これは、自己値以外を条件づけるself-censoringとも、自己値を条件づけないno self-censoringとも一般には一致しません。常時観測される個人属性、ランダム化容量、他項目のshadow signalsをCにまとめ、項目固有のVが現在評価を条件づけた後に公開指標から独立になるというshadow exclusionを置きます。これにtarget-specific bridgeを組み合わせ、分布全体ではなく平均や裾確率を直接回復します。

## Slide 04: 本研究は方法論を実験で検証し、プラットフォーム増幅へ拡張する

貢献は三つです。第一に、容量制約付きmultiple selectionを、構造的ゼロと項目間依存をもつ多変量MNARとして定式化し、対象汎関数のノンパラメトリック識別条件を示します。第二に、同じ標本から全項目の私的評価であるoracleと公開評価であるpublicを取得し、容量Kをランダム化してcrowd-outと回復性能を検証します。第三に、発展的な実験として、random、helpfulness、生成AIランキングを比較し、投稿段階の歪みが表示アルゴリズムによって増幅されるかを検証します。

## Slide 05: 既知なのは極端性、未解明なのは経験間の競争である
...
## Slide 06: Strategic sample selectionを欠測パターンへ写像する
...
## Slide 07: 選択方程式はgeneralized Roy modelに含まれる

査読上の位置づけを明確にします。各候補に公開効用があり、その上位K件を選ぶ選択方程式は、capacity constraintを伴うgeneralized Roy modelに含まれます。d'Haultfoeuille自身もshadow IVの適用例として未観測部門をもつRoy modelを挙げています。したがって、Roy modelへshadow-variable法を適用すること自体を方法論的新規性とは主張しません。貢献は、レビュー生成をexact top-Kの多変量MNARとして実装し、構造的ゼロ、cross-item crowd-out、oracle/public比較、Kとランキングのランダム化を一つの設計で検証する点です。

## Slide 08: 既存の多変量MNARはtop-Kの支持集合と両立しない
...
## Slide 09: 公開効用はpositive WOMとnegative WOMの二経路をもつ
...
## Slide 11: 命題1：Top-Kは自己単調性とcrowd-outを生む
...
## Slide 12: 命題2：公開容量が小さいほど選択効用は高くなる
...
## Slide 13: 実験仮説：U字型の裾過剰とJ字型の非対称を区別する
...
## Slide 14: 観測データは完全ケースを含まないpattern dataである
...
## Slide 15: Supported blockが観測可能な同時分布を定める
...
## Slide 16: 識別対象は支持される周辺分布と汎関数である
...
## Slide 17: 定理1：完全ケース法は構造的ゼロにより適用できない
...
## Slide 18: 定理1：supported blockを超える結合分布は非識別である
...
## Slide 20: Target-specific representer bridgeを明示する
...
## Slide 21: 定理2：対象汎関数はfull lawなしに識別できる
...
## Slide 22: 5段階評価ではbridge存在条件をrankで確認できる
...
## Slide 23: Model 1：top-Kの下ではshadow exclusionを追加仮定する
...
## Slide 24: 発展：潜在変数はsupported blocksを結ぶ追加構造である
...
## Slide 25: 定理3：測定モデルはsupported blocksを接続する
...
## Slide 26: 実験1：oracleとpublicを同一標本から取得する
...
## Slide 27: 実験1：shadow測定時点と検証範囲を分ける
...
## Slide 28: Oracleとnaiveを同じperson-average targetで比較する
...
## Slide 29: Representer bridgeの有限標本回復をoracleで評価する
...
## Slide 30: 実験2はランキングをランダム化して増幅を識別する
...
## Slide 31: 結論：多変量MNARの対象汎関数を回復し、レビューで検証する

本研究はまず、exact top-Kレビューを構造的ゼロとcross-item crowd-outをもつ多変量MNARとして定式化します。この支持制約の下では完全ケースが存在せず、full joint lawは一般に非識別です。一方、shadow exclusionとtarget-specific representerが存在すれば、平均と裾確率はfull lawなしにノンパラメトリックに識別できます。実験ではKとランキングを別々にランダム化し、選択機構、oracleに対する回復性能、アルゴリズム増幅を分けて評価します。選択方程式自体はgeneralized Roy modelに含まれるため、shadow-variable法のRoy応用ではなく、dependent multivariate MNARの定式化、識別境界、ランダム化検証を貢献とします。
...
## Slide 33: 付録A：スカラー欠測IVのノンパラメトリック識別
...
## Slide 34: 付録B：Zhao--Shaoのconditional shadow表現
...
## Slide 39: 付録E：inverse-selection bridgeによるfull-law識別
...
## Slide 42: 付録G：self-censoringの支持条件まで比較する
...
## Slide 43: 付録G：exact top-Kの支持集合との相違
...
## Slide 47: 付録I：有限サポートのboundsを計算する
...
## Slide 48: 付録J：Model 3はweak-but-many signalsを用いる
...
## Slide 49: 付録J：有限サポートではrankとrangeを区別する
