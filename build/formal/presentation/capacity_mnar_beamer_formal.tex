...
\subtitle{容量制約付きgeneralized Roy selectionのMNAR表現とレビュー実験}
...
% 2 ---------------------------------------------------------------------------
\begin{frame}[shrink=6]{容量制約付きレビューはgeneralized Roy型の多変量MNARである}
...
\scriptsize
\begin{enumerate}
\item generalized Roy型のmultiple selectionを，構造的ゼロをもつ多変量MNARとして表現する。
\item 平均評価・極端評価率をtarget-specific bridgeによりノンパラメトリックに識別する。
...
レビュー生成をgeneralized Roy型MNARとして実装し，
...
% 19 --------------------------------------------------------------------------
\begin{frame}[shrink=6]{結論：Roy型選択の対象汎関数を直接回復する}
\begin{enumerate}
\item \textbf{位置づけ}\quad
選択方程式はgeneralized Roy modelに含まれ，shadow-variable法のRoy応用自体は既知である。
\item \textbf{レビューへの貢献}\quad
